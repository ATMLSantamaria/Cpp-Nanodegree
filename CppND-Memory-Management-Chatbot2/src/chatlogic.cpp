...
ChatLogic::ChatLogic()
{
    ...
    //_chatBot = new ChatBot("../images/chatbot.png"); ->not any more here
...
ChatLogic::~ChatLogic()
{
    ...
    // delete chatbot instance -> chat logic not owner of chatBot
    // delete all nodes -> not any more
    // delete all edges -> not any more
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //std::cout << "aqui esta parentNode = " << parentNode << std::endl;
                            
                            // create new edge
    
                            auto edge = std::make_unique<GraphEdge>(id);
                            edge->SetChildNode((*childNode).get()); //This and the next line do the same since * in unique ptr is overloaded
                            edge->SetParentNode((*parentNode).get());
                            _edges.push_back(edge.get());//chat logic is not the owner

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edge.get()); //aqui esta la trasferencia de chatlogi a graphnode
    ...
    //1-Local Instance on the Stack for ChatBot
    ChatBot local_chatBot("../images/chatbot.png"); 
    //2 - add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    local_chatBot.SetChatLogicHandle(this);
    //LAs dos lineas anteriores equivalen al Constructor anterior
    
    //3-_chatBot keep a handle to this instance
    _chatBot = &local_chatBot;

    //4- Set the root node
    local_chatBot.SetRootNode(rootNode);
    
    //5-Move it to root node
